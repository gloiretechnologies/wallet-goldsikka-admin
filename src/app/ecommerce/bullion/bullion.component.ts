...
export class BullionComponent implements OnInit {
  ...
  e: any;
  filteredsubProducts2: any;
  currentGoldPricePerGram: any;
  filteredsubProducts3: any[];
  price: any;
  filteredProsilver: any[];
 ...
 showOtpScreen2: boolean;
 screen: boolean;
 screen2: boolean;
showOtpScreen3:boolean;
  myForm: FormGroup;
  myForm1: FormGroup;
  filteredProducts = [];
  filteredProducts2 = [];
 ...
 selectedQuantity = "10";
  ...
  constructor(private activate:ActivatedRoute,private api:ApiService,private fb: FormBuilder,
    private authService: AuthenticationService,private goldService:GoldService,
    ) {
...
console.log('on',r)
  ...
  getsubcat(cid) {
    ...
    this.api.get(`ecom/${cid}/subcategories`)
      ...
      .subscribe((r: any) => {
        this.sub = r;
        this.filteredsubProducts2 = [...r.filter(product => product.type == "gold")]
        this.filteredsubProducts3 = [...r.filter(product => product.type == "silver")]
        console.log('sub',this.filteredsubProducts2);
        console.log('silver',this.filteredsubProducts3);
        let sk = this.filteredsubProducts2[0].id;
        let sk2 = this.filteredsubProducts3[0].id;
         this.getpro(sk,sk2);
         this.getprosilver(sk2)
      });
  }
  onSearch(value: any) {
 ...
 let i=sk
   ...
   console.log(`second`,i2)
     this.api.get(`ecom/subcategories/${i2}/products?search=${this.searchValue}`)
      .subscribe((r: any) => {
        this.pro= r;
       this.showOtpScreen3=false;

        console.log(this.pro,this.su)
            this.filteredProsilver = [...r];
            

    this.myForm.get('filterProduct').valueChanges.subscribe(
      value => {
        this.filteredProsilver= [...r.filter(product => product.weight >= value.lower && product.weight <= value.upper )]
...
checking(e: any) {
  ...
  this.showOtpScreen3=false;
      ...
      .subscribe((r: any) => {
        this.sub = r;
        console.log('subpro',this.sub);
        this.filteredsubProducts = [...r.filter(product => product.type== "silver")]
  ...
  this.showOtpScreen2=false;
     ...
     this.api.get(`ecom/${cid}/subcategories`)
      .subscribe((r: any) => {
        this.sub = r;
         console.log('subpro',this.sub);
        this.filteredsubProducts = [...r.filter(product => product.type== "gold")]
        console.log('sub',this.filteredsubProducts);
